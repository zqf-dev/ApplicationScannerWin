...
def apkScan(inputfile, save):
    if inputfile.endswith('.apk'):
        ...
        # d 反编译 -f 覆盖 -o 输出文件路径
        cmd = 'java -jar ' + apktool + (' d -f -o %s %s' % (filePath, inputfile))
        os.system(cmd)
    ...
    try:
        ...
        # IP、web、sql注入、lib库扫描等问题for scanner
...
def fingerPrint(filePath):
    ...
    strline = f'cd {filePath}\original\META-INF && (for /r %i in (*.RSA) do @echo %i)'
    ...
    if len(rsa) > 0:
        ...
        strline = f'keytool -printcert -file {rsa}'
        out = os.popen(strline).readlines()
        result = ''
        for line in out:
            result += line
