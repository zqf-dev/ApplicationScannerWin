...
class DexLoadCheck(Base):
    def scan(self):
        ...
        if getSysType() == 'mac':
            strline = cmdString('grep -r "Ldalvik/system/DexClassLoader;-><init>" ' + self.appPath)
        elif getSysType() == 'win':
            strline = cmdString(f'cd {self.appPath} | (findstr /s "Ldalvik/system/DexClassLoader;-><init>" *.smali)')
