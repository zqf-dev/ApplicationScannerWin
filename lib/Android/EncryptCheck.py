...
class EncryptCheck(Base):
    def scan(self):
        ...
        if getSysType() == 'mac':
            strline = cmdString('grep -r "Ljavax/crypto/Cipher;->getInstance(Ljava/lang/String;)Ljavax/crypto/Cipher;" ' + self.appPath)
        elif getSysType() == 'win':
            strline = cmdString(f'cd {self.appPath} | (findstr /s "Ljavax/crypto/Cipher;->getInstance(Ljava/lang/String;)Ljavax/crypto/Cipher;" *.smali)')
